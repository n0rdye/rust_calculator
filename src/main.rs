...

{
    ...
    ui.on_number_click({
        move |int|{
            ...
            let len_of_opers = (opers.len() as u32 - 1) as usize;
            if num_regx.is_match(&opers[(opers.len() as u32 - 1) as usize]){
                if opers[len_of_opers] == "0"{
                    opers[len_of_opers] = int.to_string();
                }
                else {
                    let cur_val = &opers[(opers.len() as u32 - 1) as usize].to_string();
                    ...
                    "=" => {
                        ...
                        while let Some(num) = m2.pop() {
                            match String::as_str(&num) {
                                "+" => {
                                    let num: u32 = opers[i+1].parse().expect("Not a valid number");
                                    let last_num: u32 = opers[i-1].parse().expect("Not a valid number");
                                    sum = last_num + num;
                                    dbg!(sum);
                                }
                                "-" => {
                                ...
                                "0"=>{
                                    let num: u32 = num.parse().expect("Not a valid number");
                                    let sum = &mut sum;
                                    *sum = num;
                                }
                                _=>{print!("none")}
